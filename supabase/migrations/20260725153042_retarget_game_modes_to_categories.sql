...
-- A game mode is now a subject, or a mix of all of them.
alter type game_mode rename to game_mode_old;

create type game_mode as enum (
  'geography', 'history', 'science', 'space', 'human-world', 'mixed'
);

-- No rounds have been played yet, so there is nothing to translate.
alter table game_rounds
  alter column mode type game_mode using mode::text::game_mode;
...
begin
  ...
  -- A single-subject round may only contain questions on that subject.
